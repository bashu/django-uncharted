...
class InstanceField(Field):
    ...
    def render_field(self, name, value, attrs=None):
        ...
        if self.readonly is True:
            output.append("var %(attname)s = %(name)s.%(attname)s;" % {
                'name': name, 'attname': self.attname})

        else:
            output.append("var %(attname)s = new AmCharts.%(class)s();" % {
                'name': name, 'attname': self.attname, 'class': value.get_internal_type()})

        for field in iter(value._meta.fields):
            if not field.has_changed(field.get_default(), getattr(value, field.attname)):
                continue
            output.append(field.render_field(self.attname, getattr(value, field.attname)))
